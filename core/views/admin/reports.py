...
def export_projects_excel(request):
    ...
    for row, project in enumerate(projects, 2):
        ...
        worksheet.cell(
            row=row,
            column=4).value = project.type.name if project.type else ""
        worksheet.cell(
            row=row,
            column=5).value = project.status.name if project.status else ""
        manager_name = f"{
            project.manager.last_name} {
            project.manager.first_name}" if project.manager else ""
        ...
        worksheet.cell(
            row=row, column=9).value = float(
            project.budget) if project.budget else 0
        worksheet.cell(row=row, column=10).value = float(
            project.actual_cost) if project.actual_cost else 0
    ...
    response['Content-Disposition'] = f'attachment; filename="projects_{
...
def export_employees_excel(request):
    ...
    for row, employee in enumerate(employees, 2):
        ...
        worksheet.cell(
            row=row,
            column=3).value = employee.position.name if employee.position else ""
        worksheet.cell(
            row=row,
            column=4).value = employee.department.name if employee.department else ""
        ...
        worksheet.cell(
            row=row,
            column=8).value = "Активен" if employee.is_active else "Неактивен"
    ...
    response = HttpResponse(
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = f'attachment; filename="employees_{
        datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
    workbook.save(response)
    return response
...
def export_employees_csv(request):
    ...
    writer = csv.writer(response, delimiter=';')
    writer.writerow(["ID", "ФИО", "Должность", "Отдел",
                    "Email", "Телефон", "Дата найма", "Статус"])
...
def export_tasks_excel(request):
    ...
    for row, task in enumerate(tasks, 2):
        ...
        worksheet.cell(
            row=row,
            column=2).value = task.project.name if task.project else ""
    ...
    response = HttpResponse(
        content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    response['Content-Disposition'] = f'attachment; filename="tasks_{
        datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
    workbook.save(response)
    return response
